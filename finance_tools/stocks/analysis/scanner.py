...
class StockScanner:
    ...
    def scan(self, symbol_to_df: Dict[str, pd.DataFrame], criteria: StockScanCriteria, scanner_configs: Optional[Dict[str, Any]] = None) -> List[StockScanResult]:
        ...
        for symbol, df in symbol_to_df.items():
            ...
            if criteria.column not in df.columns:
                self.logger.warning(f"⚠️ Skipping {symbol} - Column '{criteria.column}' not found in DataFrame")
                continue
    ...
    def _build_indicator_configs(self, criteria: StockScanCriteria, scanner_configs: Optional[Dict[str, Any]] = None) -> Dict[str, IndicatorConfig]:
    ...
    def _derive_suggestion(self, df: pd.DataFrame, criteria: StockScanCriteria, indicator_configs: Dict, indicator_details: Dict) -> Tuple[StockSuggestion, float, Dict, Dict]:
