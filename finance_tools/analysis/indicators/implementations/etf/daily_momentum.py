# finance_tools/analysis/indicators/implementations/etf/daily_momentum.py
    ...
    def calculate(
        ...
        if "date" in result.columns and len(result.index) > 0:
            ...
            for w in windows:
                ...
                if (
                    pct_col not in result.columns
                    or result.at[last_idx, pct_col] is None
                    or pd.isna(result.at[last_idx, pct_col])
                ):
        ...
        for w in windows:
            ...
            result[per_day_col] = pd.Series(
                [float("nan")] * len(result), index=result.index
            )
            if last_idx is not None and pct_col in result.columns:
                val = result.at[last_idx, pct_col]
                result.at[last_idx, per_day_col] = (
                    (val / float(w)) if pd.notnull(val) else float("nan")
                )
        ...
        for w, wgt in weights.items():
            ...
            if pd.notnull(val):
                calculation_details.append(
                    f"  Period {w}d: {float(val):.6f} × {wgt} = {float(val) * wgt:.6f}"
                )

        calculation_details.extend(
            [
                "",
                "Step 1: Weighted Average = Sum(Value × Weight) / Sum(Weights)",
                f"        = {agg:.6f} / {total_w:.4f}",
                f"        = {raw:.6f}",
                "",
                "Step 2: Final Contribution = Raw Score × Indicator Weight",
                f"        = {raw:.6f} × {config.weight:.2f}",
                f"        = {contribution:.6f}",
                "",
                "Note: This represents the per-day rate of change",
            ]
        )
